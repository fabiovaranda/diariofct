<?php
	...
	if (isset($_POST['submit'])){
		...
		if ($_FILES['file1'] != ""){
			$ok = $uf->upload($_FILES['file1']);
			if ($ok == -1)
				$erro = true;
			else
				inserirFicheiro($_FILES['file1'], $idDiario);
		}
		if ($_FILES['file2'] != ""){
			$ok = $uf->upload($_FILES['file2']);
			if ($ok == -1)
				$erro = true;
			else
				inserirFicheiro($_FILES['file2'], $idDiario);
		}
		if (!$erro)
			...
			<div class='large-12 columns'>
				...
				<div class='row' style='position:relative; margin-top:2%'>	
					<div class='large-12 panel columns'>
						<form action='inicio_2.php' method='post'>
							<div class='row'>
								<div class='large-12 columns'>
									 ...
									 if(mysql_num_rows($resFD)>0){
										while($rowFD = mysql_fetch_object($resFD)){
											echo "<div class='large-6 columns'>
												<a href='ficheiros/$rowFD->ficheiro' target='_blank'>$rowFD->ficheiro</a>
											</div>";
										}										
									 }
